...
def _get_python_lib(repository_ctx, python_bin):
    ...
    if _is_windows(repository_ctx):
        _fail("_get_python_lib does not work on Windows yet. " + 
              "Specify the %s environment variable instead." % _PYTHON_LIB_PATH)
    ...
    result = _execute(repository_ctx, [_get_bash_bin(repository_ctx), "-c", cmd])
